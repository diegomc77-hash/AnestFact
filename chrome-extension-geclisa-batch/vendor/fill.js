...
if(!D||!D.getElementById('8054')){
  // Foja en el documento actual (sin iframe anidado) — p.ej. tras navegación SPA
  try{
...
// Extensión batch: token inyectado en __AFG_GECLISA_TOKEN (sin prompt). Bookmarklet: prompt.
var tok=null;
try{
...
if(!tok){
  tok=prompt('AnesFact — pegá el TOKEN GECLISA (un solo uso):');
...
function rellenar(d){
  ...
  if(d.horaInicio&&setId('8061',fmtHoraGeclisa(d.horaInicio)))ok++; // Hora inicio (con segundos)
  if(d.horaFin&&setId('8063',fmtHoraGeclisa(d.horaFin)))ok++;       // Hora fin (con segundos)
