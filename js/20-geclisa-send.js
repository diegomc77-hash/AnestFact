...
/**
 * Marca una intervención como enviada a GECLISA.
 * Nota: es estado LOCAL (automatización o marca manual). No consulta Geclisa.
...
function afMarkEnviadoGeclisa(intervId, opts){
  ...
  if(opts.toast!==false&&typeof toast==='function'){
    toast(via==='manual'||via==='manual_lista'||via==='manual_foja'||via==='manual_resumen'
      ?'Marcada enviada a GECLISA (manual) ✓'
      :'Marcada Enviado a GECLISA ✓✓');
  }
  try{
...
/** Quita la marca local enviado_geclisa (vuelve a borrador). No toca Geclisa. */
function afUnmarkEnviadoGeclisa(intervId, opts){
  opts=opts||{};
  var id=String(intervId||'').trim();
  if(!id||typeof S==='undefined'||!S.intervs){
  ...
  if(typeof saveIntervsToStorage==='function')saveIntervsToStorage();
  if(typeof syncAutoPushDebounced==='function')syncAutoPushDebounced();
  if(opts.toast!==false&&typeof toast==='function')toast('Marca GECLISA quitada (sigue en borrador)');
  try{
    if(typeof renderHome==='function'&&document.getElementById('view-home')&&
       document.getElementById('view-home').classList.contains('active')){
      renderHome();
...
/**
 * Toggle manual desde lista / foja.
 * Marca o quita "enviado_geclisa" — solo estado en AnesFact.
 */
function afToggleEnviadoGeclisaManual(intervId, ev, via){
  ...
  via=via||'manual';
  if(it.estado==='enviado_geclisa'){
    if(!confirm('¿Quitar la marca “Enviado a GECLISA”?\n(Solo afecta AnesFact; no borra nada en Geclisa.)'))return false;
    afUnmarkEnviadoGeclisa(id,{via:via});
  }else{
    if(!confirm('¿Marcar como enviada a GECLISA?\nEs una marca en AnesFact (no verifica Geclisa en vivo).'))return false;
    afMarkEnviadoGeclisa(id,{via:via,toast:true});
  }
  try{
