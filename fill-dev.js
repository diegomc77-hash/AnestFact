// AnesFact -> GECLISA filler — MODO DESARROLLO (fill-dev.js)
// Usar SOLO con AnesFact en localhost + bookmark "Rellenar GECLISA DEV"
// Los datos se leen con clave DEV_DNI — no pisa los de producción
(function(){
...
toast.style.cssText='position:fixed;bottom:20px;right:20px;background:#ffb400;color:#000;padding:12px 18px;border-radius:10px;font-size:14px;font-family:sans-serif;z-index:99999;box-shadow:0 4px 12px rgba(0,0,0,.3)';
toast.textContent='\u23f3 DEV: cargando datos de AnesFact...';
...
function cargarDatos(k){
  ...
  xhrGet(url,function(rows){
    if(!rows||!rows.length){
      rmToast();
      alert('Sin datos DEV para este paciente.\n1) Abrí AnesFact en localhost (no GitHub)\n2) Cargá el paciente\n3) Tocá "Enviar a GECLISA"\n4) Usá el bookmark DEV (no el de producción).');
      return;
    }
    try{
...
function rellenar(d){
  ...
  if(d.mantenimiento&&String(d.mantenimiento).trim()&&setId('8123',d.mantenimiento))ok++;
