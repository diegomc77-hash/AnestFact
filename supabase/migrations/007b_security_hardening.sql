-- AnesFact 007b — Endurecimiento de seguridad (SIN shares / SIN lectura de fojas ajenas)
-- Ejecutar en Supabase → SQL Editor (como dueño del proyecto)
-- Orden: después de 001–006.
-- NO ejecutar el 007 original: usar este 007b.
--
-- Incluye: sanatorios, expires_at, triggers anti auto-pro, RLS sync solo dueño,
--          af_assert_plan, af_admin_set_plan.
-- Excluye: anesfact_shares, af_is_share_partner, af_admin_link_share,
--          acceso admin a sync de otros (eso se corta también en app + 008).
...
-- ═══════════════════════════════════════════════════════════
-- 2) Trigger INSERT: forzar plan=demo, rol=user (anti auto-pro/admin)
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- 3) RLS en anesfact_datos (sync SOLO dueño; GECLISA temporal; help admin)
-- ═══════════════════════════════════════════════════════════
...
-- Legacy open policy (pre-RLS / tests): anula cualquier endurecimiento si queda
DROP POLICY IF EXISTS acceso_publico ON public.anesfact_datos;
DROP POLICY IF EXISTS "acceso_publico" ON public.anesfact_datos;

-- Auth: sync solo del dueño (admin NO lee sync ajeno por REST).
-- Help solo admin. Bridge GECLISA (no sync/help) si no venció.
...
CREATE POLICY datos_update_auth ON public.anesfact_datos
  ...
  USING (
    (clave LIKE 'anesfact_sync_%' AND owner_id = auth.uid())
...
-- ═══════════════════════════════════════════════════════════
-- 4) RPC: assert plan (cliente consulta; servidor = fuente de verdad)
-- ═══════════════════════════════════════════════════════════
...
-- Admin: setear plan (Free/Pro/etc.) + sanatorios por defecto
...
-- 5) Notas
-- ═══════════════════════════════════════════════════════════
-- - NO crea shares ni permite a admin leer sync ajeno por REST.
-- - af_admin_get_user_sync / af_admin_set_user_sync del 006 siguen existiendo
--   hasta el 008 (REVOKE/DROP) + quitar “Ver fojas” en la app.
-- - Token GECLISA de un solo uso = 008 + cambio fill.js / enviarAGeclisa.
-- - Firma certificada, crypto localStorage, export con clave, sesiones = app + 008.
