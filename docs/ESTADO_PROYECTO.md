...
- 2026-08-27 — Sub-lote 1 SISalud print (PWA **12.39**): migración 012 + header A4 3 columnas + pie ADAARC condicional + sala/cama texto SISalud. Esperando confirmación en Pages (`cache:'no-store'`). Select ∩ permitidos y append al plan Huerta = **lote 2**, no este.

## Qué se hizo (más reciente primero)

- 2026-08-27 — Sub-lote 1 SISalud (PWA **12.39**). SQL **012** en el proyecto linked: Misericordia / Córdoba / San Roque → `sin_sistema` / `sisalud` / `desarrollado=true` + meta header. Print: PNG oficial Misericordia; compose Córdoba y San Roque (`oficial: false`). Pie ADAARC omitido en foja impresa SISalud; topbar app intacto. Aero selects sin tocar. No se clonó `imprimir-aero.js`. No se tocó el select ni `sanatorios_permitidos`.
...
- Sub-lote 1 SISalud print (12.39): esperando Pages. Lote 2: `#f-san` ∩ `sanatorios_permitidos` + append de los 3 hospitales **solo** al plan Huerta.
- Dock 2 filas + tamaños (12.38): **cerrado en Pages**.
- Wizard valoración 6 pasos (12.37): esperando e2e de Diego en Pages (viaja en el SW 12.38/12.39). Token de prueba propio.
...
- Instituciones: catálogo permanente en git + Supabase (**010** + **011** + **012**). Habilitar resto = `sanatorios_permitidos`, no re-sembrar. Plan Huerta aún dice `Clínica Allende` (string legado; no tocado).
...
- Print SISalud: misma `imprimir-aero.js` (no clonar). Header 3 columnas (PNG oficial o compose). Pie ADAARC solo en papel SISalud se omite; app no. Sala/cama texto libre solo SISalud. Select de 40 no. Append de hospitales solo al plan Huerta (lote 2).
