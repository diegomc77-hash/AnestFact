...
- 2026-08-27 — Lote 2 SISalud select (PWA **12.40**): `#f-san` = desarrollado ∩ permitidos; 3 hospitales append **solo** a Huerta. Esperando Pages (flujo real, no consola). Admin ve los 5 desarrollado.

## Qué se hizo (más reciente primero)

- 2026-08-27 — Lote 2 (PWA **12.40**). Select armado desde catálogo `desarrollado`. Admin: los 5. Huerta: Aero + Mayo + Misericordia + Córdoba + San Roque. SQL **013** solo su fila (array quedó en 7; Allende/Privada Córdoba siguen en el array, no en el select). `DEFAULTS.pro` y `af_admin_set_plan` no se tocaron. Allende fuera.
...
- Instituciones: catálogo **010** + **011** + **012** + **013** (013 = solo Huerta). Habilitar resto = `sanatorios_permitidos` + `desarrollado`, no re-sembrar. String legado `Clínica Allende` sigue en el array de Huerta; no sale en el select.
...
- Cuidado: «Guardar plan» Pro en el panel admin (`af_admin_set_plan`) **pisa** `sanatorios_permitidos` con el default de 4 nombres. No usarlo sobre Huerta hasta que el RPC preserve extras.
